...
const SubjectCategoryMaster = () => {
  ...
  const fetchSubjectCategories = async () => {
    setListLoading(true);
    try {
      const response = await masterApi.getSubjectCategoryList({
        searchStr: searchQuery,
        pageNumber: currentPage - 1,
      });
      setSubjectCategories(response.data.responseModelList || []);
      setTotalCount(response.data.count || 0);
    } catch (error) {
      console.error("Error fetching subject categories:", error);
      toast.error("Failed to load subject category list");
    } finally {
  ...
  const handleEdit = (category: any) => {
    reset({
      id: category.id,
      subjectCategoryName: category.subjectCategoryName,
    });
  ...
  const handleDelete = async (id: string) => {
    if (
      !window.confirm("Are you sure you want to delete this subject category?")
    )
      return;

    try {
      await masterApi.deleteSubjectCategory(id);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-800 tracking-tight">
              Subject Category <span className="text-primary">Details</span>
            </h1>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            >
              <TextInput
                ...
                textLable="Subject Category Name"
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                >
                  ...
                  {control._formValues.id ? "UPDATE CATEGORY" : "SAVE CATEGORY"}
                </button>
                <button
                  type="button"
                  onClick={() => reset({ id: "", subjectCategoryName: "" })}
                ...
                <tbody className="divide-y divide-slate-50">
                  {listLoading ? (
                    ...
                    <tr>
                      ...
                      >
                        No subject categories found matching your search.
                      </td>
                    </tr>
                  )}
                </tbody>
...
export default SubjectCategoryMaster;
