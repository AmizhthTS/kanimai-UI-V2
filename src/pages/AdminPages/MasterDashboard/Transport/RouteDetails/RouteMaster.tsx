...
const RouteMaster = () => {
  ...
  const fetchRoutes = async () => {
    setListLoading(true);
    try {
      const response = await masterApi.getRouteList({
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await masterApi.deleteRoute(id);
    ...
    <div className="space-y-6 pb-10 animate-in fade-in duration-700">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-800 tracking-tight">
              Route <span className="text-primary">Details</span>
            </h1>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 flex flex-col h-full">
        <div className="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button
            onClick={() => navigate("/admin/master/bus-route/add")}
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-slate-50/50">
                      ...
                      <th className="px-6 py-4 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest w-40">
                        Number of Stops
                      </th>
                    ...
                    {routes.map((route, index) => {
                      ...
                      try {
                        if (route.boardingPoints) {
                          const points = JSON.parse(route.boardingPoints);
                          stopsCount = Array.isArray(points) ? points.length : 0;
                        }
                      } catch (e) {
                        stopsCount = 0;
                      }
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="space-y-2 flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-black px-2 py-0.5 bg-blue-50 text-blue-600 rounded-md uppercase tracking-wider">
                              {stopsCount} Stops
                            </span>
                          </div>
                        ...
                        <button
                          onClick={() =>
                            navigate(`/admin/master/bus-route/edit/${route.id}`)
                          }
