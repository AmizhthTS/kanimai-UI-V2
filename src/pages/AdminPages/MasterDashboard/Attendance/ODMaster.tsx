...
const ODMaster = () => {
  ...
  const fetchODs = async () => {
    setListLoading(true);
    try {
      const response = await masterApi.getODList({
        searchStr: searchQuery,
        pageNumber: currentPage - 1,
      });
      setOds(response.data.responseModelList || []);
      setTotalCount(response.data.count || 0);
    } catch (error) {
      console.error("Error fetching OD categories:", error);
      toast.error("Failed to load OD list");
    } finally {
  ...
  const onFormSubmit = async (data: any) => {
    setLoading(true);
    try {
      await masterApi.saveOD(data);
      toast.success(
        data.id ? "OD Category updated successfully" : "OD Category saved successfully",
      );
      reset({ id: "", odName: "" });
      fetchODs();
    } catch (error) {
      console.error("Error saving OD category:", error);
      toast.error("Failed to save OD category");
    } finally {
      setLoading(false);
    }
  };

  const handleEdit = (od: any) => {
    reset({
      id: od.id,
      odName: od.odName,
    });
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Are you sure you want to delete this OD category?")) return;

    try {
      await masterApi.deleteOD(id);
      toast.success("OD Category deleted successfully");
      fetchODs();
    } catch (error) {
      console.error("Error deleting OD category:", error);
      toast.error("Failed to delete OD category");
    }
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-800 tracking-tight">
              OD Category <span className="text-primary">Details</span>
            </h1>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="bg-slate-50/50 px-6 py-4 border-b border-slate-100">
              <h3 className="font-bold text-slate-800 flex items-center gap-2 text-sm">
                ...
                {control._formValues.id ? "Update OD Category" : "Add OD Category"}
              </h3>
            </div>
            ...
            <form onSubmit={handleSubmit(onFormSubmit)} className="p-6 space-y-5">
              <TextInput
                ...
                errors={errors}
                name="odName"
                textLable="OD Category Name"
                placeholderName="Enter OD Category Name"
                requiredMsg="OD Category Name is required"
                labelMandatory
              />
        ...
        <div className="lg:col-span-8">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 flex flex-col h-full">
            <div className="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h3 className="font-bold text-slate-800 text-sm">OD Category List</h3>
              </div>
                ...
                <tbody className="divide-y divide-slate-50">
                  {listLoading ? (
                    ...
                    <tr>
                      ...
                      >
                        No OD categories found matching your search.
                      </td>
                    </tr>
                  )}
                </tbody>
