...
const SectionMaster = () => {
  ...
  const fetchSections = async () => {
    ...
    try {
      ...
      setSections(response.data.responseModelList || []);
  ...
  const onFormSubmit = async (data: any) => {
    setLoading(true);
    try {
      await masterApi.saveClassSection(data);
      toast.success(
        data.id ? "Section updated successfully" : "Section saved successfully",
      );
      reset({ id: "", sectionName: "" });
      fetchSections();
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await masterApi.deleteClassSection(id);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-800 tracking-tight">
              Section <span className="text-primary">Details</span>
            </h1>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="bg-slate-50/50 px-6 py-4 border-b border-slate-100">
              <h3 className="font-bold text-slate-800 flex items-center gap-2 text-sm">
                ...
                {control._formValues.id ? "Update Section" : "Add New Section"}
              </h3>
            </div>
            ...
            >
              <TextInput
                ...
                errors={errors}
                name="sectionName"
                textLable="Section Name"
                placeholderName="e.g. Section A"
                requiredMsg="Section Name is required"
                labelMandatory
              />
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                >
                  ...
                  {control._formValues.id ? "UPDATE SECTION" : "SAVE SECTION"}
                </button>
                <button
                  type="button"
                  onClick={() => reset({ id: "", sectionName: "" })}
