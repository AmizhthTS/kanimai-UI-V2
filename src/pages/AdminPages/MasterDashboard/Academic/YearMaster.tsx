...
const YearMaster = () => {
  ...
  const fetchYears = async () => {
    ...
    try {
      ...
      setYears(response.data.responseModelList || []);
      setTotalCount(response.data.count || 0);
    } catch (error) {
      console.error("Error fetching years:", error);
      toast.error("Failed to load year list");
    } finally {
  ...
  const onFormSubmit = async (data: any) => {
    setLoading(true);
    try {
      await masterApi.saveYear(data);
      toast.success(
        data.id ? "Year updated successfully" : "Year saved successfully",
      );
      reset({ id: "", yearName: "" });
      fetchYears();
    } catch (error) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-800 tracking-tight">
              Year <span className="text-primary">Details</span>
            </h1>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="bg-slate-50/50 px-6 py-4 border-b border-slate-100">
              <h3 className="font-bold text-slate-800 flex items-center gap-2 text-sm">
                ...
                {control._formValues.id ? "Update Year" : "Add New Year"}
              </h3>
            </div>
            ...
            >
              <TextInput
                ...
                name="yearName"
                textLable="Year Name"
                placeholderName="e.g. 1 Year"
                requiredMsg="Year Name is required"
                labelMandatory
              />
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                >
                  ...
                  {control._formValues.id ? "UPDATE YEAR" : "SAVE YEAR"}
                </button>
                <button
                  type="button"
                  onClick={() => reset({ id: "", yearName: "" })}
        ...
        <div className="lg:col-span-8">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 flex flex-col h-full">
            <div className="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h3 className="font-bold text-slate-800 text-sm">Year List</h3>
              </div>
